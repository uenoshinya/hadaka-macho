...
export default function MealsPage() {
  ...
  const anyInput = Object.values(meals).some((m) => m.content.trim());
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl bg-[#2C1A0E] px-6 py-5 text-center">
        ...
        <p className="mt-1 text-xs text-[#FFF8EC]/60">食べたものを入力 → 同期 → トレーナーが分析</p>
      </div>
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              {meal.content.trim() && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#D4A017] font-semibold">入力済み — トレーナーが分析します</span>
                </div>
              )}
            </div>
          </section>
      ...
      {/* 全体メモ */}
      <section>
        <label className="block text-sm font-semibold text-[#5C3D11] mb-2">📝 今日の食事メモ（任意）</label>
        <textarea
          value={note}
